...
class SingleInvoices extends Component
{
    ...
    public function print($id)
    {
        ...
        ///////////// Note  When We Use Redirect We Cant  Use Compact So We Send The Attribute in the next metho
        // Take A look on Print Page 
    ...
    {
        // get the first doctor withe this id that sended to yo
        $doctor_id = Doctor::with('section')->where('id', $this->doctor_id)->first();
    // by the relation between doctor and section we get the section when we select our ductor outomaticlly 
    ...
    {
        // get the price of the service in the moment that we select the service 
        // take the id and go to search and take the first service 
    ...
    public function edit($id){
        ...
        // after calling the objct and store object`s attribute i will send them to edit page  
        $this->show_table = false;
        ...
        if($this->type == 1){
            ...
            try {
                ...
                // في حالة الاضافة
                    ...
                    $single_invoices->invoice_type = 1; // single invoice
                    $single_invoices->invoice_date = date('Y-m-d');
                    $single_invoices->patient_id = $this->patient_id;
                    $single_invoices->doctor_id = $this->doctor_id;
                    $single_invoices->section_id = DB::table('section_translations')->where('name', $this->section_id)->first()->section_id;
                    ...
                    $single_invoices->total_with_tax = $single_invoices->price -  $single_invoices->discount_value + $single_invoices->tax_value;
                    $single_invoices->type = $this->type;
                    $single_invoices->invoice_status = 1;//vpending
                    $single_invoices->save();
                     // we finished a single invoice save 


                     // now we will send it too fundAccount table
                     $fund_accounts = new FundAccount();
                     $fund_accounts->date=date('y-m-d');//the current date
                     $fund_accounts->invoice_id=$single_invoices->id;// the invoice that we have save it before 
                     $fund_accounts->Debit=$single_invoices->total_with_tax;// send the total price to save it as a debit
                     $fund_accounts->credit=0.00;
                     $fund_accounts->save();
        ...
        // في حالة كانت الفاتورة اجل
            ...
            try {
                ...
                if($this->updateMode){
                    ...
                    $patient_accounts = PatientAccounnt::where('invoice_id',$this->single_invoice_id)->first();
